...
typedef struct {
	...
	// feed forward
	float feedfwd_0order_K; // multiply by W
	float feedfwd_1order_K; // multiply by derivation of W
	float feedfwd_1order_T; // Ks/(Ts+1)
	float feedfwd_1order_Integrator;
...
float MOTOR_ControllerUpdate( PID_CONTROLLER* pid, float requiredValue, float feedback, uint32_t uTime);
