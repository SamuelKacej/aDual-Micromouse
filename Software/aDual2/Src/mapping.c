...
uint8_t MAPPING_LookForWalls( uint8_t sensorSelect){
	...
	if(sensorSelect == 0){
		...
		if(callCnt[3] != 0 ){
			//left
			const uint8_t leftAvg = sensorsDist[3]/callCnt[3];
		...
		if(callCnt[2] != 0){
			//right
			const uint8_t rightAvg = sensorsDist[2]/callCnt[2];
		...
		if(callCnt[0] != 0 && callCnt[5] != 0){
			//forward
			const uint8_t fwdAvgR = sensorsDist[0]/callCnt[0];
			const uint8_t fwdAvgL = sensorsDist[5]/callCnt[5];
				// if one of them show there is no wall (0), then return there is no wall
			if(fwdAvgL < MAPPING_WALL_TRESHOLD_FRONT && fwdAvgR < MAPPING_WALL_TRESHOLD_FRONT )
				wall.WALL.front = 1;
		}
